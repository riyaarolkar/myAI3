...
export function ExploreFilters({ filters, onFilterChange, onApplyFilters }: ExploreFiltersProps) {
  ...
  return (
    <div className="bg-gray-900/50 backdrop-blur-lg border border-white/10 rounded-2xl p-4">
      <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Select value={filters.priceRange} onValueChange={(v) => handleChange("priceRange", v)}>
                ...
                <SelectContent className="bg-gray-900 border-white/10">
                  {PRICE_RANGES.map((p) => (
                    <SelectItem key={p.value} value={p.value} className="text-white hover:bg-gray-800">
                      {p.label}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select value={filters.brand} onValueChange={(v) => handleChange("brand", v)}>
                ...
                <SelectContent className="bg-gray-900 border-white/10">
                  {BRANDS.map((b) => (
                    <SelectItem key={b.value} value={b.value} className="text-white hover:bg-gray-800">
                      {b.label}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select value={filters.bagType} onValueChange={(v) => handleChange("bagType", v)}>
                ...
                <SelectContent className="bg-gray-900 border-white/10">
                  {BAG_TYPES.map((t) => (
                    <SelectItem key={t.value} value={t.value} className="text-white hover:bg-gray-800">
                      {t.label}
                    </SelectItem>
                  ))}
                </SelectContent>
